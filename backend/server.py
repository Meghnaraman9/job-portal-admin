...
def get_jobs():
    ...
    params = []
    if search:
        q += " AND (j.title LIKE ? OR j.company LIKE ?)"
...
def create_job():
    ...
    c.execute('INSERT INTO jobs (title,company,category,location,type,status,salary,posted_by) VALUES (?,?,?,?,?,?,?,1)',
              (d['title'], d['company'], d['category'], d['location'],
               d.get('type', 'Full-time'), d.get('status', 'active'), d.get('salary', '')))
    conn.commit()
    conn.close()
...
def get_apps():
    ...
    params = []
    if search:
        q += " AND (u.name LIKE ? OR j.title LIKE ?)"
        params += ['%' + search + '%', '%' + search + '%']
